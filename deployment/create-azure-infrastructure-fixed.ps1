...
try {
    ...
    az webapp config appsettings set --name $BackendAppName --resource-group $ResourceGroupName --settings "DATABASE_URL=$sqlConnectionString" "STORAGE_CONNECTION_STRING=$storageConnectionString" "ENVIRONMENT=$Environment" "FRONTEND_URL=https://$FrontendAppName.azurewebsites.net"

    # 11. Configura App Settings per Frontend
    Write-Host "⚙️ Configurazione App Settings Frontend..." -ForegroundColor $Green
    az webapp config appsettings set --name $FrontendAppName --resource-group $ResourceGroupName --settings "REACT_APP_API_URL=https://$BackendAppName.azurewebsites.net" "NODE_ENV=production"

    # 12. Abilita CORS per Backend
    Write-Host "🌐 Configurazione CORS..." -ForegroundColor $Green
    az webapp cors add --name $BackendAppName --resource-group $ResourceGroupName --allowed-origins "https://$FrontendAppName.azurewebsites.net"
