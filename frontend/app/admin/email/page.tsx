...
import EmailConfigList from '@/components/admin/EmailConfigList';
import EmailMonitoringStatus from '@/components/admin/EmailMonitoringStatus';
import EmailLogViewer from '@/components/admin/EmailLogViewer';
import RealTimeEmailMonitor from '@/components/admin/RealTimeEmailMonitor';
import NotificationSystem, { NotificationProps } from '@/components/admin/NotificationSystem';
import { 
  EnvelopeIcon, 
  Cog6ToothIcon, 
  ChartBarIcon,
...
export default function EmailManagementPage() {
        ...
        <Card>
          <CardContent className="text-center py-8">
            ...
            <p className="text-gray-600 dark:text-gray-400">
              You need admin privileges to access email management.
            </p>
          </CardContent>
        </Card>
      </div>
  ...
  const loadConfigs = async () => {
    ...
    try {
      ...
      showNotification('error', 'Failed to load email configurations');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleConfigSaved = (config: EmailConfig) => {
    ...
    showNotification('success', `Email configuration "${config.name}" ${editingConfig ? 'updated' : 'created'} successfully`);
  };

  const handleConfigDeleted = (configName: string) => {
    loadConfigs();
    showNotification('success', `Email configuration "${configName}" deleted successfully`);
  };

  const handleTestResult = (result: any, configName: string) => {
    showNotification(
      result.success ? 'success' : 'error',
      `${configName}: ${result.message}`
    );
  };
      ...
      case 'realtime':
        return (
          <div className="space-y-6">
            ...
            <RealTimeEmailMonitor refreshInterval={5000} />
          </div>
        );
